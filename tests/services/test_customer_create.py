...
class TestCustomerCreate:
    ...
    def test_when_customer_does_not_exists_and_all_details_correct_creates_customer(self, database):
        ...
        customer = CustomerCreate.call(self.token, self.details)
    ...
    def test_when_customer_does_not_exists_and_with_extra_details_creates_customer(self, database):
        ...
        customer = CustomerCreate.call(self.token, {**self.details, 'extra-attr': 'something' })

        assert customer is not None
        assert Customer.find_by_email(self.email) == customer

    def test_when_customer_exists_and_no_token_provided_returns_existing_customer(self, database):
    ...
    def test_when_customer_details_are_invalid_does_not_create_customer(self, database, invalid_detail):
        ...
        with pytest.raises(InvalidCustomer):
            customer = CustomerCreate.call(self.token, {**self.details, **invalid_detail} )
    ...
    def test_when_customer_details_are_valid_and_optional_present_creates_customer(self, database, optional_details):
        ...
        customer = CustomerCreate.call(self.token, {**self.details, **optional_details} )
